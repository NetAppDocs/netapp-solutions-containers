...
.       Install the F5 Application Services 3 extension to allow BIG-IP systems to accept configurations in JSON instead of imperative commands. Go to link:https://github.com/F5Networks/f5-appsvcs-extension/releases[F5 AS3 GitHub repository^], and download the latest RPM file.
...
+
image::redhat-openshift-109.png[iApps upload]

.	Confirm that the AS3 extension is installed successfully.
+
image::redhat-openshift-110.png[AS3 install validation]
...
+
image::redhat-openshift-111.png[Create VXLAN profile]
...
+
image::redhat-openshift-112.png[Create VXLAN tunnel]
...
+
image::redhat-openshift-113.png[Create Self IP for VXLAN]
...
+
image::redhat-openshift-114.png[Create BIG-IP Partition]
+
...
+
image::redhat-openshift-116.png[F5 CIS Info tile in OperatorHub]

.	On the Install operator screen, leave all default parameters, and click Install.
+
image::redhat-openshift-117.png[Install F5 CIS operator]

.	It takes a while to install the operator.
+
image::redhat-openshift-118.png[F5 CIS Operator install progress]
...
+
image::redhat-openshift-119.png[Create F5BigIpCtlr]
...
+
image::redhat-openshift-120.png[Validate F5 CIS pods]
+
...
+
image::redhat-openshift-121.png[Validate the creation of BIG-IP virtual servers for corresponding service type LoadBalancer]



// NetApp Solutions restructuring (jul 2025) - renamed from containers/rh-os-n_LB_F5BigIP.adoc
