...
spec:
  providerConfig:
    ...
    s3:
      ...
      # rootCA: <base64-encoded root CA certificate>
  providerCredentials:
    accessKeyID:
      valueFromSecret:
        key: accessKeyID
...
NOTE: The sample PostgreSQL PVCs use RWO access mode for both NAS and SAN. Kubernetes requires each PVC to request an access mode; Trident does not assign RWX or RWO defaults based on the storage protocol. RWX is supported for NAS, and SAN can support RWX with additional configuration. For more information, refer to the link:https://docs.netapp.com/us-en/trident/trident-protect/learn-about-trident-protect.html[Trident Protect documentation].
