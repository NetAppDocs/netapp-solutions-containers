...
spec:
  providerConfig:
    ...
    s3:
      ...
      secure: "false" # Use "true" (the default) for HTTPS in production.
      skipCertValidation: "true" # Use "false" (the default) for production.
...
Before deleting the PostgreSQL workload, wait for the backup to reach the `Completed` state:

[source,console]
....
tp get backup -n postgres
....

After the backup is complete, delete the PostgreSQL application resources and ensure that the PVCs and pod objects are deleted from the `postgres` namespace.
...
Delete the application objects and the PVCs for postgres from the postgres namespace.
