...
spec:
  providerConfig:
    ...
    s3:
      ...
      secure: "false" <use true (default) for https for production>
      skipCertValidation: "true" < use false (default) for production>
      //use trusted certificate for production
  providerCredentials:
    accessKeyID:
      valueFromSecret:
        key: accessKeyID
...
First, delete the postgres application and ensure that the PVCs and pod objects are deleted from the namespace "postgres".
