...
Container applications running in a VKS cluster are managed as Kubernetes workloads in worker node namespaces. It is important to protect both the application metadata and the persistent disks so that, if they are lost or corrupted, you can recover them.
...
* link:https://docs.netapp.com/us-en/trident/trident-get-started/quickstart.html[Trident 25.06]
* link:https://docs.netapp.com/us-en/trident/trident-protect/trident-protect-installation.html[Trident Protect 25.06]
...
First, create and label the `trident-protect` namespace. The `enforce` label is required — without it, the Kubernetes pod security admission controller will block Trident Protect pods from starting. The `audit` and `warn` labels are recommended so that any future policy violations are logged and surfaced before they cause failures.

[source,cli]
....
kubectl create namespace trident-protect
kubectl label namespace trident-protect \
...
kubectl label namespace trident-protect pod-security.kubernetes.io/audit=privileged --overwrite
kubectl label namespace trident-protect pod-security.kubernetes.io/warn=privileged --overwrite
....
...
Here is the workflow for creating an AppVault CR for ONTAP S3:
...
IMPORTANT: The manifest below uses HTTPS with certificate validation enabled, which is required for production. If your ONTAP S3 endpoint uses a certificate signed by a public CA already trusted by the cluster, no additional certificate configuration is needed. If it uses a self-signed or internal CA certificate, provide the base64-encoded CA certificate using the `caCert` field as shown in the manifest. See the lab-only variant at the end of this section if you need an HTTP configuration for isolated non-production testing.
...
# you can also provide string values instead of base 64 encoded values
#stringData:
...
[%collapsible]
====
In this example, data protection for the sample postgres application, installed previously, is performed. The application is installed in the namespace postgres. For details on installation, refer to the section link:/vsphere-kubernetes-service/deploy-vks-workloads.html[Deploy VKS workloads using NetApp Storage].

NOTE: Trident supports RWX access mode as the default mode for NAS protocols. In Block Volume mode for SAN (iSCSI, NVMe/TCP and FC), the default access mode is RWO. RWX Access mode is also supported for SAN protocols, but it requires additional configuration. For more information, refer to the link:https://docs.netapp.com/us-en/trident/trident-protect/learn-about-trident-protect.html[Trident Protect documentation].
...
Before deleting the application, verify that the backup you plan to restore from is in the `Completed` state. An in-progress or failed backup cannot be used to recover a deleted workload.
...
We have a new hourly backup, created using the schedule, that is now available for the postgres-app in the postgres namespace. We will use this backup to restore the application to the new namespace postgres2.
...
NOTE: To get the path for the backup, use the command `tp get backups -n postgres -o yaml | grep <backup name>`.
...
Before deleting the application, verify that the snapshot you plan to restore from is in the `Completed` state. An in-progress or failed snapshot cannot be used to recover a deleted workload.
