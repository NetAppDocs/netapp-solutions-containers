...
. ontap-nas: One PVC maps to one PV, which is a dedicated FlexVolume.
...
+
One PVC maps to one PV, which is a qtree on a shared FlexVolume.
...
. ontap-san: One PVC maps to one PV, which is a LUN on a dedicated FlexVolume.

. ontap-san-economy: One PVC maps to one PV, which is a LUN on a shared FlexVolume. You can have multiple LUNs in the shared FlexVolume (default is 100, configurable between 50 and 200 LUNs/PVs per FlexVolume).
