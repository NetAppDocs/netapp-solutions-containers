...
+
Before running the Helm install, create and label the `trident` namespace. The `enforce` label is required — without it, the Kubernetes pod security admission controller will block Trident pods from starting. The `audit` and `warn` labels are recommended so that any future policy violations are logged and surfaced before they cause failures.
+
[source,cli]
----
kubectl create namespace trident
kubectl label namespace trident \
...
[source,cli]
----
helm install trident netapp-trident/trident-operator --version 100.2606.0 --namespace trident --kubeconfig=/path/to/your-kubeconfig-file
----
...
[source,cli]
----
helm install trident netapp-trident/trident-operator --version 100.2606.0 --namespace trident --set tridentDebug=true --kubeconfig=/path/to/your-kubeconfig-file
----
====
+
...
+
Ensure that you have labeled the `trident` namespace with the required pod security labels before installing Trident manually.
...
NOTE: If Trident pods fail to start with a `PodSecurity` admission error, the pod security labels may not have been applied to the `trident` namespace before installation. Use `--overwrite` to reapply them, then verify the pods start.
