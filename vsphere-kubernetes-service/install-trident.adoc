...
+
Before running the Helm install, create and label the `trident` namespace. The `enforce=privileged` label is required because Trident runs privileged workloads. Without it, the Kubernetes pod security admission controller will block Trident pods from starting.
+
[source,cli]
----
